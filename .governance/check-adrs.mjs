 ...
 *   7. No file under <adr-dir>/_drafts/ carries a pre-assigned "# ADR-NNNN" header —
 *      numbers are claimed only by scripts/land-adr.mjs at land time (ADR-0043). This
 *      is the direct guard against the failure mode that produced two independent
 *      ADR-0036 files on 2026-07-08: an agent hand-writing a numbered header while
 *      drafting, before checking what else might be in flight.
 *
 * USAGE:
 ...
 * directly too (known-issues.md's Cowork git-lock issue was fixed 2026-07-08, ADR-0044 — clear-then-commit via scripts/lib/git-lockclear.sh; note it was NOT actually fixed on 2026-07-02 as this comment previously and incorrectly claimed).
 */
...
function resolveAdrDir() {
  const explicit = process.argv.includes('--dir') ? argValue('--dir', '') : null;
  if (explicit) return path.resolve(process.cwd(), explicit);
  for (const candidate of ['.governance/adr', 'adr', 'work-system/adr']) {
    const resolved = path.resolve(process.cwd(), candidate);
    if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) return resolved;
  }
  console.error(
    '✗ No ADR directory found. Tried .governance/adr, adr, work-system/adr relative to cwd.\n' +
      '  Pass --dir <path> explicitly.'
  );
  process.exit(1);
}

const ADR_DIR = resolveAdrDir();
...
const allFiles = fs
  ...
  .filter((f) => fs.statSync(path.join(ADR_DIR, f)).isFile()); // excludes _drafts/ etc.
const adrFiles = [];
...
// --- drafts: catch hand-numbered drafts before they become a second collision ---
const DRAFTS_DIR = path.join(ADR_DIR, '_drafts');
if (fs.existsSync(DRAFTS_DIR)) {
  for (const f of fs.readdirSync(DRAFTS_DIR)) {
    if (!f.endsWith('.md') || f === 'README.md') continue;
    const text = fs.readFileSync(path.join(DRAFTS_DIR, f), 'utf8');
    if (/^#\s+ADR-\d{4}\b/m.test(text)) {
      fail(
        `_drafts/${f}`,
        'draft carries a numbered "# ADR-NNNN" header — numbers are assigned only by ' +
          'scripts/land-adr.mjs at land time (ADR-0043). Run land-adr.mjs on it, or remove ' +
          'the number if this was hand-written.'
      );
    }
  }
}
